...
def get_args():
    ...
    parser.add_argument("initial", help="CSV for test suite")
...
def print_latex_command(lang, category, d, mut=""):
    template = "\\newcommand{{\\{lang}cov{mut}{category}{metric}}}{{\\nnum{{{num}}}\\xspace}}"
    for k, v in d.items():
...
def print_res(lang, initial, comb, latex, mut, compiler):
    ...
    initial_dict = get_dict_format(
        compute_perc(initial['total'], 'line'),
        compute_perc(initial['total'], 'function'),
        compute_perc(initial['total'], 'branch'))
    ...
    change_dict = get_dict_format(
        comb_dict['line'] - initial_dict['line'],
        comb_dict['function'] - initial_dict['function'],
        comb_dict['branch'] - initial_dict['branch'])
    abs_dict = get_dict_format(
        compute_abs_diff(comb['total'], initial['total'], 'line'),
        compute_abs_diff(comb['total'], initial['total'], 'function'),
        compute_abs_diff(comb['total'], initial['total'], 'branch'))
    ...
    if latex:
        ...
        for category, d in categories:
            print_latex_command(lang, category, d, mut)
...
def main():
    ...
    initial = read_csv(args.initial, whitelist)
    ...
    if args.increasepkg or args.increasecls:
        print()
        increase = compute_increase(initial, comb, args.increasecls)
        increase_view = [(
            v['branch_perc'],
